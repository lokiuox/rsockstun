...
func main() {
	...
	serverFlags.Usage = func() {
		...
		fmt.Printf("%s server [-listen <listenAddr>] [options]\n", os.Args[0])
		fmt.Println("Options:")
	...
	clientFlags.Usage = func() {
		...
		fmt.Printf("%s client -connect <connectAddr> [-socks <socksAddr>] [-proxy <proxyURI>] [options]\n", os.Args[0])
		fmt.Println("Options:")
	...
	flag.Usage = func() {
		fmt.Println("fsockstun - forward socks5 server/client")
		fmt.Println()
		serverFlags.Usage()
		clientFlags.Usage()
	...
	case "server":
		...
		clientListener(*listen, *certificate)
